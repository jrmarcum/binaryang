...
import { heapTypeNameToType, Type } from '../../../src/wabt-ts/core/types.ts';
...
describe('resolveNames leaves no unresolved name-var (standing guard)', () => {
  ...
  function moduleSurvivors(module: Module) {
    ...
    for (const f of module.functions) for (const b of f.body.children) survivors(b, 'body', out);
    return out;
  }
  ...
  it('…and label names DO survive it, which is the other half of the rule', () => {
    ...
    for (const f of module.functions) walk(f.body.children);
  ...
  it('holds across the whole spec testsuite', () => {
    ...
    for (const e of entries) {
      ...
      for (const cmd of script.commands) {
        ...
        for (const f of cmd.scriptModule.module.functions) {
