...
const PINNED = {
  ...
  global: { onlyW: ['loc'], onlyB: [], differ: ['init'] },
  table: {
    onlyW: ['loc', 'init', 'elemType', 'limits'],
    onlyB: ['type', 'initial', 'max'],
    differ: [],
  },
  ...
  data: { onlyW: ['loc', 'kind', 'memoryVar'], onlyB: ['memory', 'passive'], differ: ['offset'] },
...
describe('S6 step 5 item 6 — Module / WasmModule convergence ratchet', () => {
  ...
  it('records the distance', () => {
    ...
    assertEquals([count('onlyW'), count('onlyB'), count('differ')], [46, 29, 15]);
  });
});
