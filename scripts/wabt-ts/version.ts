 ...
 *
 * Shared version helpers for the release scripts. The wabt-ts versioning rule
 * matches sibling binaryen-ts: each sub-version maxes at 9 before rolling into
 * the next greater segment. So patch 9 → minor +1 (patch reset to 0), minor 9
 * → major +1 (minor + patch reset to 0).
 *
 *   1.0.6 → 1.0.7
 *   1.0.9 → 1.1.0
 *   1.9.9 → 2.0.0
