...
export class ExprVisitor {
  ...
  private dispatch(e: Expr): Result {
    switch (e.kind) {
      ...
      case 'nop':            return this.d.onNopExpr?.(e) ?? Result.Ok;
      case 'unreachable':    return this.d.onUnreachableExpr?.(e) ?? Result.Ok;
      case 'const':          return this.d.onConstExpr?.(e) ?? Result.Ok;
      case 'local.get':      return this.d.onLocalGetExpr?.(e) ?? Result.Ok;
      case 'global.get':     return this.d.onGlobalGetExpr?.(e) ?? Result.Ok;
      case 'memory.size':    return this.d.onMemorySizeExpr?.(e) ?? Result.Ok;
      case 'table.size':     return this.d.onTableSizeExpr?.(e) ?? Result.Ok;
      case 'ref.null':       return this.d.onRefNullExpr?.(e) ?? Result.Ok;
      case 'ref.func':       return this.d.onRefFuncExpr?.(e) ?? Result.Ok;
      case 'atomic_fence':   return this.d.onAtomicFenceExpr?.(e) ?? Result.Ok;
      case 'data.drop':      return this.d.onDataDropExpr?.(e) ?? Result.Ok;
      case 'elem.drop':      return this.d.onElemDropExpr?.(e) ?? Result.Ok;
      case 'rethrow':        return this.d.onRethrowExpr?.(e) ?? Result.Ok;
      case 'code_metadata':  return this.d.onCodeMetadataExpr?.(e) ?? Result.Ok;

      // --- Single child then callback ---
      case 'return': {
        if (e.value !== undefined) { const r = this.dispatch(e.value); if (r === Result.Error) return r; }
        return this.d.onReturnExpr?.(e) ?? Result.Ok;
      }
      case 'drop': {
        const r = this.dispatch(e.value); if (r === Result.Error) return r;
      ...
      case 'unary': {
        const r = this.dispatch(e.operand); if (r === Result.Error) return r;
      ...
      case 'memory.grow': {
        const r = this.dispatch(e.delta); if (r === Result.Error) return r;
        return this.d.onMemoryGrowExpr?.(e) ?? Result.Ok;
      }
      case 'table.get': {
        const r = this.dispatch(e.index); if (r === Result.Error) return r;
        return this.d.onTableGetExpr?.(e) ?? Result.Ok;
      }
      case 'load': {
        const r = this.dispatch(e.address); if (r === Result.Error) return r;
      ...
      case 'throw_ref': {
        const r = this.dispatch(e.exnref); if (r === Result.Error) return r;
        return this.d.onThrowRefExpr?.(e) ?? Result.Ok;
      }

      // --- Two children then callback ---
      case 'binary': {
        let r = this.dispatch(e.left); if (r === Result.Error) return r;
        r = this.dispatch(e.right); if (r === Result.Error) return r;
      ...
      case 'store': {
        let r = this.dispatch(e.address); if (r === Result.Error) return r;
        r = this.dispatch(e.value); if (r === Result.Error) return r;
      ...
      case 'atomic_notify': {
        let r = this.dispatch(e.address); if (r === Result.Error) return r;
        r = this.dispatch(e.count); if (r === Result.Error) return r;
        return this.d.onAtomicNotifyExpr?.(e) ?? Result.Ok;
      }
      case 'table.set': {
        let r = this.dispatch(e.index); if (r === Result.Error) return r;
        r = this.dispatch(e.value); if (r === Result.Error) return r;
        return this.d.onTableSetExpr?.(e) ?? Result.Ok;
      }
      case 'table.grow': {
        let r = this.dispatch(e.initValue); if (r === Result.Error) return r;
        r = this.dispatch(e.delta); if (r === Result.Error) return r;
      ...
      case 'simd_load_lane': {
        let r = this.dispatch(e.address); if (r === Result.Error) return r;
        r = this.dispatch(e.vec); if (r === Result.Error) return r;
      ...
      case 'br_if': {
        if (e.value !== undefined) { const r = this.dispatch(e.value); if (r === Result.Error) return r; }
        const rc = this.dispatch(e.cond); if (rc === Result.Error) return rc;
      ...
      // --- Three children ---
      case 'select': {
        let r = this.dispatch(e.val1); if (r === Result.Error) return r;
        r = this.dispatch(e.val2); if (r === Result.Error) return r;
        r = this.dispatch(e.cond); if (r === Result.Error) return r;
        return this.d.onSelectExpr?.(e) ?? Result.Ok;
      }
      case 'memory.copy': {
        let r = this.dispatch(e.dest); if (r === Result.Error) return r;
        r = this.dispatch(e.src); if (r === Result.Error) return r;
        r = this.dispatch(e.size); if (r === Result.Error) return r;
      ...
      case 'table.copy': {
        let r = this.dispatch(e.dest); if (r === Result.Error) return r;
        r = this.dispatch(e.srcOffset); if (r === Result.Error) return r;
        r = this.dispatch(e.size); if (r === Result.Error) return r;
        return this.d.onTableCopyExpr?.(e) ?? Result.Ok;
      }
      case 'table.fill': {
        let r = this.dispatch(e.start); if (r === Result.Error) return r;
        r = this.dispatch(e.value); if (r === Result.Error) return r;
        r = this.dispatch(e.size); if (r === Result.Error) return r;
      ...
      case 'atomic_rmw_cmpxchg': {
        let r = this.dispatch(e.address); if (r === Result.Error) return r;
        r = this.dispatch(e.expected); if (r === Result.Error) return r;
        r = this.dispatch(e.replacement); if (r === Result.Error) return r;
        return this.d.onAtomicRmwCmpxchgExpr?.(e) ?? Result.Ok;
      }
      case 'atomic_wait': {
        let r = this.dispatch(e.address); if (r === Result.Error) return r;
        r = this.dispatch(e.expected); if (r === Result.Error) return r;
        r = this.dispatch(e.timeout); if (r === Result.Error) return r;
        return this.d.onAtomicWaitExpr?.(e) ?? Result.Ok;
      }
      case 'ternary': {
        let r = this.dispatch(e.a); if (r === Result.Error) return r;
        r = this.dispatch(e.b); if (r === Result.Error) return r;
        r = this.dispatch(e.c); if (r === Result.Error) return r;
      ...
      case 'quaternary': {
        ...
        r = this.dispatch(e.b); if (r === Result.Error) return r;
        r = this.dispatch(e.c); if (r === Result.Error) return r;
        r = this.dispatch(e.d); if (r === Result.Error) return r;
        return this.d.onQuaternaryExpr?.(e) ?? Result.Ok;
      }

      // --- Calls: visit args in order, then callback ---
      case 'call': {
        for (const arg of e.args) { const r = this.dispatch(arg); if (r === Result.Error) return r; }
        return this.d.onCallExpr?.(e) ?? Result.Ok;
      }
      case 'call_indirect': {
        for (const arg of e.args) { const r = this.dispatch(arg); if (r === Result.Error) return r; }
        const r = this.dispatch(e.callee); if (r === Result.Error) return r;
      ...
      case 'block': {
        let r = this.d.beginBlockExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.body); if (r === Result.Error) return r;
        return this.d.endBlockExpr?.(e) ?? Result.Ok;
      }
      case 'loop': {
        let r = this.d.beginLoopExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.body); if (r === Result.Error) return r;
        return this.d.endLoopExpr?.(e) ?? Result.Ok;
      }
      case 'if': {
        let r = this.dispatch(e.cond); if (r === Result.Error) return r;
        r = this.d.beginIfExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.then_); if (r === Result.Error) return r;
        r = this.d.afterIfTrueExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.else_); if (r === Result.Error) return r;
        return this.d.endIfExpr?.(e) ?? Result.Ok;
      }
      case 'try': {
        let r = this.d.beginTryExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.body); if (r === Result.Error) return r;
        if (e.delegate !== undefined) {
          return this.d.onDelegateExpr?.(e) ?? Result.Ok;
        }
        for (const [i, c] of e.catches.entries()) {
          r = this.d.onCatchExpr?.(e, c, i) ?? Result.Ok; if (r === Result.Error) return r;
          r = this.visitExprList(c.body); if (r === Result.Error) return r;
        }
        return this.d.endTryExpr?.(e) ?? Result.Ok;
      }
      case 'try_table': {
        let r = this.d.beginTryTableExpr?.(e) ?? Result.Ok; if (r === Result.Error) return r;
        r = this.visitExprList(e.body); if (r === Result.Error) return r;
