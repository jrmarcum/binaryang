...
interface KwBare   { readonly tt: TokenType }
interface KwOp     { readonly tt: TokenType; readonly op: number }
interface KwType   { readonly tt: TokenType; readonly vt: Type }
type KwInfo = KwBare | KwOp | KwType;

function bare(tt: TokenType): KwBare   { return { tt }; }
function op(tt: TokenType, o: number): KwOp  { return { tt, op: o }; }
function vt(tt: TokenType, v: Type): KwType  { return { tt, vt: v }; }
...
function isDigit(c: number): boolean    { return (CHAR_CLASS[c] ?? 0) >= 8; }
function isHexDigit(c: number): boolean { return ((CHAR_CLASS[c] ?? 0) & 4) !== 0; }
function isKeyword(c: number): boolean  { return ((CHAR_CLASS[c] ?? 0) & 2) !== 0; }
function isIdChar(c: number): boolean   { return ((CHAR_CLASS[c] ?? 0) & 1) !== 0; }
...
export class WastLexer {
  ...
  constructor(source: LexerSource | string) {
    this.src = source instanceof LexerSource
      ? source
      : new LexerSource(source);
  }
  ...
  private peek(): number  { return this.src.peek(); }
  private read(): number  { return this.src.read(); }
  private get cursor(): number { return this.src.offset; }

  private matchChar(c: number): boolean {
    if (this.peek() === c) { this.read(); return true; }
    return false;
  }
  ...
  private opcodeToken(tt: TokenType, o: number): Token {
    return { loc: this.getLocation(), tokenType: tt, opcode: o as unknown as typeof Opcode[keyof typeof Opcode] };
  }
  ...
  getToken(): Token {
    while (true) {
      ...
      switch (c) {
        ...
        case 0x20: case 0x09: case 0x0d: case 0x0a: // space, tab, CR, LF
          this.readWhitespace();
          continue;
        ...
        case 0x36: case 0x37: case 0x38: case 0x39: // '1'-'9'
  ...
  private readBlockComment(): boolean {
    ...
    while (true) {
      const c = this.read();
      if (c === EOF) { this.error('EOF in block comment'); return false; }
  ...
  private readLineComment(): boolean {
    while (true) {
      ...
      if (c === 0x0a) { this.newlineChar(); return true; }
    }
  }
  ...
  private readWhitespace(): void {
    while (true) {
      const c = this.peek();
      if (c === 0x20 || c === 0x09 || c === 0x0d) { this.read(); continue; }
      if (c === 0x0a) { this.read(); this.newlineChar(); continue; }
      break;
    }
  }
  ...
  private getStringToken(tt: TokenType): Token {
    ...
    outer: while (true) {
      ...
      if (c === 0x5c) { // backslash escape
        const e = this.read();
        switch (e) {
          case 0x74: case 0x6e: case 0x72: case 0x22: case 0x27: case 0x5c:
            break; // valid single-char escapes
          case 0x30: case 0x31: case 0x32: case 0x33: case 0x34:
          case 0x35: case 0x36: case 0x37: case 0x38: case 0x39:
          case 0x61: case 0x62: case 0x63: case 0x64: case 0x65: case 0x66:
          case 0x41: case 0x42: case 0x43: case 0x44: case 0x45: case 0x46:
            if (isHexDigit(this.peek())) { this.read(); break; }
          ...
          case 0x75: { // \u{XXXX}
            this.tokenStart = this.cursor - 2;
            if (!this.matchChar(0x7b)) { this.error('bad escape'); hasError = true; break; }
            let scalar = 0;
            while (isHexDigit(this.peek())) {
              const digit = parseInt(String.fromCharCode(this.read()), 16);
              scalar = (scalar << 4) | digit;
              if (scalar >= 0x110000) { this.error('bad escape'); hasError = true; break outer; }
            }
            if (!this.matchChar(0x7d)) { this.error('bad escape'); hasError = true; break; }
            if ((scalar >= 0xd800 && scalar < 0xe000)) { this.error('bad escape'); hasError = true; }
            break;
          }
          default:
            this.tokenStart = this.cursor - 2;
  ...
  private getHexNumberToken(tt: TokenType): Token {
    if (this.readHexNum()) {
      ...
      if (this.noTrailingReserved()) {
        return this.literalToken(tt, tt === TokenType.Float ? LiteralType.Hexfloat : LiteralType.Int);
      }
    }
    return this.getReservedToken();
  }
  ...
  private getIdChars(): Token {
    ...
    let hasString = false;
    while (true) {
      if (isIdChar(this.peek())) { this.read(); continue; }
      if (this.peek() === 0x22) { this.getStringToken(TokenType.Text); hasString = true; continue; }
      break;
    }
