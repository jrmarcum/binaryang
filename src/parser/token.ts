...
export function isBareToken(t: TokenType): boolean    { return t >= FIRST_BARE    && t <= LAST_BARE; }
export function isLiteralToken(t: TokenType): boolean { return t >= FIRST_LITERAL && t <= LAST_LITERAL; }
export function isOpcodeToken(t: TokenType): boolean  { return t >= FIRST_OPCODE  && t <= LAST_OPCODE; }
export function isStringToken(t: TokenType): boolean  { return t >= FIRST_STRING  && t <= LAST_STRING; }
export function isTypeToken(t: TokenType): boolean    { return t >= FIRST_TYPE    && t <= LAST_TYPE; }
export function isRefKindToken(t: TokenType): boolean { return t >= FIRST_REFKIND && t <= LAST_REFKIND; }
