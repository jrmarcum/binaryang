...
| —     | ✅ Done    | Round-3 wasmtk integration bug fixes — the first round to surface **optimization-pass** correctness bugs (rounds 1–2 were parser/encoder). (a) **`Inlining` produced an invalid block fallthru**: when a callee returns its result via `return` (rewritten to `br $wrapper`), the callee body block is typed `unreachable`, which the encoder emits with a void blocktype; once inlining nested it as a child of the result-typed wrapper, the wrapper's structural fallthrough was reachable with an empty stack and V8 rejected it ("expected 1 element on the stack for fallthru, found 0"). The inliner now appends an explicit `unreachable` to the wrapper whenever the body does not fall through with the result value. (b) **`CoalesceLocals` mis-dispatched `call_indirect`** ("function signature mismatch" at runtime): wasm evaluates `call_indirect` operands before the table index, but the CFG visited the index first, so a `local.tee` in an operand whose value feeds only the index of the same call looked dead and was eliminated — the index then read a stale local and dispatched to the wrong (wrong-signature) function. The CFG now visits `call_indirect` operands before the target, matching execution order. (c) **WAT parser** emitted export kind `"func"` instead of `"function"` for the standalone `(export "x" (func $f))` form, which corrupted the export section on encode and let `Inlining` delete the exported function; now mapped to the canonical `"function"`. 3 new regression tests (the second verified to fail without the CFG fix); 299/299 passing. Root-caused against the wasmtk team's `12_Final_Integration_Stress` fixture.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| —     | ✅ Done    | Round-4 wasmtk integration bug fix — **`try`/`catch` handler re-emitted wrapped in a spurious `block`** ([src/encoder/wasm-encoder.ts](src/encoder/wasm-encoder.ts)). The binary parser packs a multi-instruction `(catch $tag ...)` handler into an anonymous `Block` container (the same convention as the function-body frame); the Try encoder emitted it via the generic block path, wrapping it in `block ... end`. But the `catch` opcode pushes the tag's parameter values onto the catch region's stack, not the inner (void-typed) block's — so the handler's leading `local.set`s ran on an empty stack and V8 rejected the binary ("not enough arguments on the stack for local.set"). This was a bare round-trip corruption (`readBinary().emitBinary()` with no passes turned a valid binary invalid) and broke every wasic-emitted `try`/`catch`. Fix: unpack an anonymous-Block catch handler — emit its children directly into the catch region — mirroring the existing function-body-container unpacking. 1 new regression test (115-byte wabt fixture; bare round-trip stays valid + executes, verified to fail without the fix); 300/300 passing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |

| — | ✅ Done | Round-5 wasmtk integration bug fix — **the full `-Oz` pipeline produced a
dangling-stack fallthru in catch handlers that bind-then-ignore the tag's params**
([src/binary/wasm-parser.ts](src/binary/wasm-parser.ts)). Raw wasm, the bare round-trip, and every
single pass were valid in isolation; only the full pipeline failed (V8 "expected 0 elements on the
stack for fallthru, found 2") — a pass _interaction_ bisected to the second `Vacuum`. wasic emits
each catch-param bind as `nop; nop; local.set N`, where the `nop`s carry no stack value and the real
value is the param `catch` pushed. The parser seeded the catch body with a single hard-coded `I32`
`Pop` and `pop()` blindly took the top entry, so each `local.set` consumed a `nop` placeholder →
`local.set(nop)`; `CoalesceLocals` rewrote the dead binds to `drop(nop)` and the second `Vacuum`
deleted them, dropping the consumption of the values `catch` pushed and leaving them dangling at the
function tail. Fix: the catch handler seeds one typed `Pop` per tag param, and `pop()` returns the
topmost value-producing expression (skipping `none`-typed statements, leaving them in place) — so
dead binds become `drop(pop)`, which `Vacuum` correctly leaves alone. The `pop()` change surfaced a
latent WAT-parser gap: `(ref.null …)` / `(ref.func …)` / `(ref.is_null …)` had no handler and
