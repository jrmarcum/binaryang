...
- ⬚ **N1 — names lost at three hops** (divergences.md N1). Owner asked that it not be skipped, and
  DECIDED (2026-09-10): names are kept BY DEFAULT when reading and writing without optimization —
  over both upstreams, which drop them unless asked — and optimized output follows a `-g`-style
  `debugInfo` option (default off, as upstream, unless re-decided). Export and import names are the
  interface, not N1, and are already pinned. (1) wabt-ts's writer: implement the ignored
  `writeDebugNames` — a name section; (2) binaryen-ts's decoder: pre-scan for the name section and
  name entities from it, uniquified; (3) binaryen-ts's encoder: write it under `debugInfo` AND in
  `lowerBlockParams`' re-encode, or 7b(i)'s name check breaks for named modules. Then `readWat` asks
  for names. Tests: `$foo` through every route, and byte comparison with upstream
  `wat2wasm --debug-names` / `wasm-opt -g`.
