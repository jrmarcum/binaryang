...
| N6 | both       | a `name` section whose LOCAL subsection lists only the functions that HAVE local names (every clang / rustc / zig binary) re-encodes with an entry for every function — 2+ bytes, not byte-identical | DEFECT     | ⬚ found 2026-09-11 (C3). BOTH halves, one cause: N1 matched upstream `wat2wasm --debug-names`, which lists them all. 9 of 9 real-producer WASI binaries miss byte-identity on this alone; the fix is to record WHICH functions the subsection listed                     |
