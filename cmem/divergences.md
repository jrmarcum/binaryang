...
| C3 | binaryen   | binaryen-ts's decoder collects NO custom section: a decode → encode drops `producers`, `target_features`, `dylink.0`. Upstream `wasm-opt` keeps every one (appended at the end, not restored to its position) | DEFECT     | ⬚ found 2026-09-11 (C2), probed both ways. Pinned by `tests/wabt-ts/parser/custom_annotation.test.ts`; wabt-ts already keeps them, positions and all |
