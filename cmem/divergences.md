...
| C2 | wabt       | the WAT writer prints custom sections as `(@custom …)`, which the parser cannot read: `wasm2wat` → `wat2wasm` DROPS every custom section                                                             | DEFECT     | ✅ FIXED 2026-09-11: the lexer emits `LparAnn` for `(@custom` alone, the parser reads it, the writer prints the POSITION. Every other annotation is still skipped                                                                                                                                                                                                            |
...
| N6 | both       | a `name` section whose LOCAL subsection lists only the functions that HAVE local names (every clang / rustc / zig binary) re-encoded with an entry for every function — 2+ bytes, not byte-identical | DEFECT     | ✅ FIXED 2026-09-11 (found in C3): the reader records WHICH functions the section listed — `Module.localNamesListed` / `ExplicitNames.localsListed` — and each writer lists exactly those. No record still means every function, upstream's shape, so our own bytes are unchanged (baseline IDENTICAL). Real WASI binaries: 366 → **374 of 376** byte-identical, BOTH halves |
