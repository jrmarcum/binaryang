...
| [phases.md](phases.md)                     | Phase delivery plan (1–8) with status, the TS↔C++ file mapping per phase, and the per-phase gotchas (IR field names, keyword tokens, fold-form invariants, validator rules, CLI pipeline decisions); the post-v1.3.5 conformance campaign as a CLOSED snapshot (its numbers are dated, not current), and **the ONGOING post-v1.4.0 audit/hardening tranche T13 at a glance** — findings to date, the 12 unreleased user-visible fixes, the one deliberately unfixed, and the empty enumeration frontier                                                                                                                                    |
