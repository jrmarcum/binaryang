...
| [ir-convergence.md](ir-convergence.md)         | **What actually separates the two IRs**, measured against upstream binaryen rather than argued. A stack-sourced operand has no place in a tree; upstream SPILLS IT TO A LOCAL and rewrites each consumer as an explicit read. binaryen-ts already has that machinery and does not use it here; wabt-ts records the situation with a `placeholder` marker instead. Scoped in three stages.                                                                                                                                                                                                                                                                                                  |
| [divergences.md](divergences.md)               | **Every divergence from upstream wabt and binaryen, classified** — FEATURE (we support what they lack, e.g. GC text), DESIGN (a recorded S6 choice), ORACLE GAP (upstream cannot judge it), DEFECT (open, then history). Read BEFORE refactoring, optimizing, or porting an upstream pass: a faithful port can silently undo a DESIGN row.                                                                                                                                                                                                                                                                                                                                                 |
| [names.md](names.md)                           | **N1, scoped (2026-09-11): internal names.** ~43,000 in the corpus, 2 survive our round trip; the wabt-ts reader has never read one (a slice bug); upstream writes no LABEL names, so keeping them goes beyond it. The owner's rules, measurements per kind and per hop, the design constraints, and the six-step plan.                                                                                                                                                                                                                                                                                                                                                                    |
...
| [best-practices.md](best-practices.md)         | **Merged (A16), convergent rules only.** The rules **both** projects derived independently, each with both origin stories. The full enumerations stay in the wings; see the file for why that split rather than a rewrite. Includes the EIGHT ways a field change breaks silently, where a compiler-driven conversion stops, why a vocabulary must be checked against the spec, the headline S6 rule (make the defect UNREPRESENTABLE, finding where by its compensating mechanism), why the local gate must be CI's gate, why a new KIND is found first by every private `switch` default, why a node COUNT is behaviour, and why a recorded reason is a hypothesis until a probe agrees. |
...
| [testing.md](testing.md)                       | **Merged (§2.2).** How binaryang is tested, and the testing philosophy both projects reached independently: three states not two, every metric's blind spot, the four tests needing neither corpus nor oracle, and why every test's held-fixed dimension is its next blind spot. Also which gate sees WHAT: the byte gates are blind to a pass that stops firing, and `baseline` is the only thing that sees a construction defect.                                                                                                                                                                                                                                                        |
