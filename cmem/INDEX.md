...
| File                                       | What it holds                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [overview.md](overview.md)                 | What wabt-ts is, the long-term wasmtk/wasm2ts goal, **the current conformance state (all SEVEN metrics)**, the cross-runtime support matrix, the unreleased breaking changes, repo layout, the upstream C++ reference tree                                                                                                                                                                                 |
...
| [testing.md](testing.md)                   | How to run the gate; **the seven conformance metrics and what each is blind to**; **the tests that need neither a corpus nor an oracle** (differentials, code-enumerated populations, gate tests) and why the proposals the corpus LACKS are the blind spot; the wasmtk WAT corpus runner and the fact that `tests/wasmtk/` is a FROZEN SNAPSHOT (`PROVENANCE.md`); regression-test placement |
...
| [publishing.md](publishing.md)             | JSR package `@jrmarcum/wabt-ts`; tag-driven publish flow (never `deno publish` locally — OIDC provenance); the bump task + sub-version-capped-at-9 rule; CI workflows; **the UNRELEASED breaking changes waiting on a version bump**, and why `--dry-run` must be run when an exported symbol moves                                     |
| [best-practices.md](best-practices.md)     | **Method rules, each paid for by a real incident here** — parse-count is an upper bound, confirm root causes with repros not error text, producer/consumer order mismatches, invert a new guard test before trusting it, measure severity never inherit it, name the ref you measured, the proposals your corpus lacks are your blind spot, audit your own diff before the codebase. Structure adopted from the sibling wazmrt project's file of the same name. |
