...
| [publishing.md](publishing.md)             | JSR package `@jrmarcum/wabt-ts`; tag-driven publish flow (never `deno publish` locally — OIDC provenance); the bump task + sub-version-capped-at-9 rule; CI workflows; **what v1.4.0 breaks**, **why `auto-tag.yml` can tag but CANNOT publish** (and how to recover), and why `--dry-run` must be run when an exported symbol moves                                     |
