...
| [pre-merge-known-issues.md](pre-merge-known-issues.md) | **Written 2026-08-25, before the binaryang merge.** Everything MEASURED from the wabt-ts side while the repos are still separate, so nothing gets absorbed into "it was already like that": the bridge de-coarsening is **incomplete with 3 measured failing shapes and no test** (imported func / tag with `(ref $T)`, and a global of `(ref null $T)`); **five `src/` directories collide with different contents** — `src/ir` is the wabt IR in one and the binaryen IR in the other, which is the very distinction the bridge exists to translate; 21 tracked-path collisions including 7 `cmem/` files; `compilerOptions` differ in 5 of 7 but cost only **4 type errors**; `fmt.singleQuote` is opposite; both upstreams' Apache attribution obligations combine. Also records what the merge RESOLVES, and the rule that entries are MARKED when fixed, never deleted |
